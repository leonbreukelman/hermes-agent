...
def _fake_github_token(fill: str, length: int = 30) -> str:
    return _FAKE_GITHUB_PREFIX + (fill * length)


def _fake_opaque_access_token() -> str:
    return "opaque-access-" + "token-value-123456789"
...
def test_build_kwargs_rejects_generic_acp_args():
    ...
    with pytest.raises(RuntimeError, match="does not accept extra CLI/acp_args"):
        transport.build_kwargs(
            model="sonnet",
            messages=[{"role": "user", "content": "Say hello."}],
            cli_path="/tmp/claude",
            acp_args=["--dangerously-skip-permissions"],
        )
...
def test_run_raises_actionable_redacted_error_when_cli_binary_is_missing():
    transport = get_transport("claude_code")
    secret = _fake_anthropic_token("a")
...
def test_run_redacts_credential_like_values_from_nonzero_exit():
    transport = get_transport("claude_code")
    secret = _fake_anthropic_token("b")
...
def test_run_raises_clear_redacted_error_on_invalid_json_stdout():
    transport = get_transport("claude_code")
    secret = _fake_anthropic_token("c")
...
def test_timeout_terminates_claude_child_process(monkeypatch):
    transport = get_transport("claude_code")
    process = _FakePopen(
        stdout="token " + _fake_anthropic_token("d"),
        stderr="api" + "_key=" + "e" * 30,
        timeout_count=2,
        wait_times_out=True,
    )
    popen_calls = []
    ...
    assert _FAKE_ANTHROPIC_PREFIX not in message
...
def test_subprocess_does_not_inherit_anthropic_credentials(monkeypatch):
    transport = get_transport("claude_code")
    secret = _fake_anthropic_token("f")
...
def test_child_env_preserves_windows_bootstrap_vars_without_anthropic_credentials(monkeypatch):
    ...
    monkeypatch.setenv("ANTHROPIC_API_KEY", _fake_anthropic_token("w"))
    monkeypatch.setenv("CLAUDE_CODE_OAUTH_TOKEN", "oauth-token-should-not-pass")
...
def test_error_json_subtype_raises_redacted_transport_error():
    transport = get_transport("claude_code")
    secret = _fake_anthropic_token("g")
...
def test_invalid_timeout_env_var_produces_clear_error(monkeypatch):
    transport = get_transport("claude_code")
    secret = _fake_anthropic_token("i")
...
def test_error_previews_are_bounded_and_redacted():
    transport = get_transport("claude_code")
    secret = _fake_anthropic_token("j")
...
def test_provider_data_in_successful_response_is_redacted_and_bounded():
    transport = get_transport("claude_code")
    secret = _fake_anthropic_token("k")
    opaque_key = "opaque-provider-key-value-987654321"
    camel_token = _fake_opaque_access_token()
    huge_payload = "z" * 5000
...
def test_stream_json_parser_emits_assistant_deltas_only():
    secret = _fake_anthropic_token("m")
    lines = [
...
def test_stream_json_parser_rejects_malformed_lines_redacted():
    secret = _fake_anthropic_token("n")
