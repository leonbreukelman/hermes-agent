...
def switch_model(
    ...
    *,
    ...
    # failure, etc.) restore these atomically so the agent is not left with a
    # new model/provider name paired with the old client. Include the Claude
    # Code command fields because the local provider bridge mutates them too.
    _MISSING = object()
    _snapshot = {
    ...
    try:
        ...
        if api_mode == "claude_code":
            # Claude Code takes only a command override. Ignore generic ACP args
            # so callers cannot inject extra Claude CLI switches beyond Hermes'
            # fixed, safety-bounded command template.
            agent.acp_args = []
